...
for p_hil in range(1, 9): # resolución máxima 4K
...
def gabor_img(img, kernel):
    ...
    return np.sqrt(convolve(img, np.real(kernel), mode='wrap')**2 +
                   convolve(img, np.imag(kernel), mode='wrap')**2)
