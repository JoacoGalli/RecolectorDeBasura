...
def buffer_porcentaje(foto):
    ...
    img_buffer = img.imread('buffer_vacio_800_bn.jpg')
    #img_buffer = img.imread('imagen_resultante.jpg')
    ...
    mean_edge_thr = 0.96 # subir para detectar más basura

    # imágenes transformadas
    img_edge_mean_tr_bn, img_edge_mean_tr_g = mean_transform(img_edge_bn, img_buffer_bn, kernel_size, mean_edge_thr, True)  # si aca le pongo False no me pone la barrita?
    ...
    rounded_filling_percentage = round(filling_percentage, 2)
    print(f'El buffer esta un {rounded_filling_percentage}% lleno.')
    #muevo_x = (filling_percentage - 21)/ 79

    #titulo = str(filling_percentage)
    return filling_percentage
    #plt.figure(figsize = (15, 10)) #individuales para el informe
    ...
    #plt.grid(False)
    #plt.box(False)
    #plt.suptitle(f'El % de llenado del buffer es de {filling_percentage}', fontsize=16)
    #plt.savefig("buffer/plot_"+foto)

if __name__ == "__main__":
    nombres = ["captura_con_tapas3.jpg"]#["buffer_25.jpg","buffer_50.jpg","buffer_75.jpg","buffer_100.jpg"]
    for n in nombres:
    #sacar_foto(nombre)
    #buffer_porcentaje(nombre)
    #tiempo_st = time.time()
    #sacar_foto(nombre)
        x = buffer_porcentaje(n)
