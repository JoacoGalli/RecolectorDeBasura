...
if __name__ == "__main__":
    ...
    for pos in range(4):
        print(f"{pos=} y {maquina.posicion_cinta-pos}")
        time.sleep(0.8)
    ...
    if (maquina.posicion_cinta - x) > 0 :
        traslacion.clockwise = False
        traslacion.motores_status = "on"
        print(f"{maquina.posicion_media[str(x)]=}")
        print(f"{maquina.posicion_cinta_cm}")
        while not ( (maquina.posicion_media[str(x)] - 1) < maquina.posicion_cinta_cm < (maquina.posicion_media[str(x)] + 1)) :
            #print(" no salgo del while")
            maquina.ubicacion_cinta()
            time.sleep(0.2)
        traslacion.motores_status = "off"
    elif (maquina.posicion_cinta - x) < 0: 
        traslacion.clockwise = True
        traslacion.motores_status = "on"
        while not( maquina.posicion_media[str(x)] - 1 < maquina.posicion_cinta_cm < maquina.posicion_media[str(x)] + 1) :
            maquina.ubicacion_cinta()
