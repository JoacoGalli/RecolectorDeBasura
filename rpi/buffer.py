...
file_acumar = drive.CreateFile({'id':'1fwTcfb8tS8-lL-rt_DVo0_EsruGa0aSP'})
file_acumar.GetContentFile('acumar.jpeg')
img_acumar = img.imread('acumar.jpeg')

file_buffer = drive.CreateFile({'id':'1qUvY_3W5El44O7tEtAewFHhKV1nKWSBU'})
file_buffer.GetContentFile('buffer.png')
img_buffer = img.imread('buffer.png')
...
acumar_gray = rgb2gray(img_acumar)
buffer_gray = rgb2gray(img_buffer)
...
# Alternativa con filtro de Gabor

lam = 4 # lambda
pi_fraction = np.array(range(10))/10
# pi_fraction = np.array([0, 1, 2, 7, 8, 9])/10
angles = pi_fraction*np.pi
sigma = 2
sigma_x = sigma
sigma_y = sigma

kernels = [gabor_kernel(1/lam, theta = theta, sigma_x = sigma_x, sigma_y = sigma_y) for theta in angles]

# plot
plt.figure(figsize = (4, 4))

# gabor image
img_gabor = np.ones(img_main_gray.shape)
# img_gabor = np.zeros(img_main_gray.shape)
i = 0
for kernel in kernels:
    filtered_img = gabor_img(img_main_gray, kernel)
    img_gabor *= np.max(np.array([filtered_img, np.ones(img_main_gray.shape)*0.005]), 0)**0.15

    i = i + 1

# img_gabor /= len(kernels)
...
img_gabor_mean_tr, mean_edge = mean_transform(1 - img_gabor, kernel_size, mean_edge_thr, True)

garbage_img = img_gabor_mean_tr
garbage_border_img = garbage_border(garbage_img, square_img(img_acumar, offset_x = start_x, offset_y = start_y, square_size = img_main_gray.shape[0]))

# plot
plt.figure(figsize = (4, 4))

plt.subplot(1, 3, 1)
plt.imshow(1 - img_gabor, cmap = 'gray')
plt.xticks([])
plt.yticks([])
plt.grid(False)
plt.box(False)
...
plt.subplot(1, 3, 2)
plt.imshow(img_gabor_mean_tr, cmap = 'gray', vmin = 0, vmax = 1)
plt.xticks([])
plt.yticks([])
plt.grid(False)
plt.box(False)
...
plt.subplot(1, 3, 3)
plt.imshow(garbage_border_img)
plt.xticks([])
plt.yticks([])
plt.grid(False)
plt.box(False)

# Alternativa con detección de bordes según píxeles vecinos

img_hfedge, img_hf = hf_img(img_main_gray, 0.1)
...
img_hf_mean_tr, mean_edge = mean_transform(img_hfedge, kernel_size, mean_edge_thr, True)

garbage_img = img_hf_mean_tr
garbage_border_img = garbage_border(garbage_img, square_img(img_acumar, offset_x = start_x, offset_y = start_y, square_size = img_main_gray.shape[0]))
...
plt.subplot(2, 2, 1)
plt.imshow(img_hfedge, cmap = 'gray', vmin = 0, vmax = 1)
plt.xticks([])
plt.yticks([])
plt.grid(False)
plt.box(False)
...
plt.subplot(2, 2, 2)
plt.imshow(img_hf, cmap = 'gray', vmin = 0, vmax = 1)
plt.xticks([])
plt.yticks([])
plt.grid(False)
plt.box(False)
...
plt.subplot(2, 2, 3)
plt.imshow(img_hf_mean_tr, cmap = 'gray', vmin = 0, vmax = 1)
plt.xticks([])
plt.yticks([])
plt.grid(False)
plt.box(False)
...
# Alternativa con detección de bordes usando la imagen a color

color_edge_thr = 15
edge_thr = 0.1

img_edge_color, img_hilbert_color = color_hilbert_img(img_main_color, edge_thr = color_edge_thr)
img_edge, img_hilber = hilbert_img(img_main_gray, edge_thr = edge_thr)

color_mean_thr = 0.45
mean_thr = 0.75
kernel_size = 64

img_edge_color_mean_tr, img_edge_color_mean = mean_transform(img_edge_color, kernel_size = kernel_size, mean_thr = color_mean_thr, progress_bar = True)
img_edge_mean_tr, img_edge_mean = mean_transform(img_edge, kernel_size = kernel_size, mean_thr = mean_thr, progress_bar = True)

img_border_edge_color_mean_tr = garbage_border(img_edge_color_mean_tr, img_main_color)
img_border_edge_mean_tr = garbage_border(img_edge_mean_tr, img_main_color)

# plot
plt.figure(figsize = (4, 4))

plt.subplot(2, 2, 1)
plt.imshow(img_edge_color_mean_tr, cmap = 'gray', vmin = 0, vmax = 1)
plt.xticks([])
plt.yticks([])
plt.grid(False)
plt.box(False)
...
plt.subplot(2, 2, 2)
plt.imshow(img_border_edge_color_mean_tr)
plt.xticks([])
plt.yticks([])
plt.grid(False)
plt.box(False)
...
plt.subplot(2, 2, 4)
plt.imshow(img_border_edge_mean_tr)
plt.xticks([])
plt.yticks([])
plt.grid(False)
plt.box(False)
